...
/**
 * 카카오 API로 지역 코드 및 주소 요청
 * @param {*} param0
 */
const getBCodeAndAddress = async ({ latitude, longitude }) => {
    ...
    if (response.status === 200 && response.data) {
        ...
        try {
            ...
            // NOTE: 행정동 기준의 도큐먼트를 찾음
            const hCodeDocument = documents.find(
                (document) => document["region_type"] === "H"
            );
            const { address_name: address } = hCodeDocument;

            return {
                bCode: fullBCode.substring(0, 5),
                address,
            };
        } catch (_) {
            return null;
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                async ({ coords: { latitude, longitude } }) => {
                    const { bCode, address } = await getBCodeAndAddress({
                        latitude,
                        longitude,
                    });
